...
namespace Feri.MS.Parts.I2C.DAC
{
    internal class MCP4725Helper
    {
        internal I2cDevice I2cController { get; set; }
        internal MCP4725 Part { get; set; }
        internal int Address { get; set; }
        internal int ReferenceCount { get; set; } = 0;
    }
    class MCP4725 : IDisposable
    {
        ...
        private static Dictionary<int, MCP4725Helper> _initialized { get; set; } = new Dictionary<int, MCP4725Helper>();
        public int Address { get; set; } = 0;

        private MCP4725(int address)
        {
            Address = address;
        }

        public static MCP4725 Create(int address = 0x48, string i2cControllerDeviceId = null)
        {
            ...
            if (!_initialized.ContainsKey(address))
            {
                ...
                _part = new MCP4725(address);
                MCP4725Helper helper = new MCP4725Helper();
